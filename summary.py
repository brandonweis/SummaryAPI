...
class Summary(object):
    ...
    def get_best_sentence(self, paragraph, sentences_dic):
        # improvement
        # get best 2 sentence or 3, depending on the setting
        # if the content is one long paragraph, need to select a optimum number of best sentence
    ...
    def compute_summary(self):
        # Build the sentences dictionary
        sentences_dic = self.get_senteces_ranks(self.content)

        # Build the summary with the sentences dictionary
        self.summary = self.get_summary(self.title, self.content, sentences_dic)

        # compute summary ratio
        self.summary_ratio = 100 - (100 * (len(self.summary) / (len(self.title) + len(self.content))))
    ...
    def print_result(self):
        ...
        print ""
        print "Original Length %s" % (len(self.title) + len(self.content))
        print "Summary Length %s" % len(self.summary)
        print "Summary Ratio: %s" % (self.summary_ratio)
